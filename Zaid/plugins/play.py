...
async def play(event):
    ...
    if (
        ...
        return await event.client.send_file(chat_id, Config.CMD_IMG, caption="**Beri Saya Nama Lagu yang Ingin Anda Mainkan**\n\n **Contoh**: `/play Febby Putri Usik`", buttons=btnn)
    elif replied and not replied.audio and not replied.voice or not replied:
        botman = await event.reply("**🔄 Memproses Permintaan... Harap Tunggu!**")
        ...
        if search == 0:
            ...
            if hm == 0:
                ...
                caption = f"⌛ **Menambahkan Antrian Ke** #{pos}\n\n💡 **Judul:** [{songname}]({url})\n**⏰ Durasi:** `{duration}`\n👥 **Permintaan Oleh:** {from_user}"
                await botman.delete()
        ...
        botman = await edit_or_reply(event, "➕ Mengunduh File...")
...
async def vc_end(event, perm):
    ...
    if chat_id in QUEUE:
        try:
            await call_py.leave_group_call(chat_id)
            clear_queue(chat_id)
            await event.reply("**Streaming Berakhir**")
        except Exception as e:
...
async def vplay(event):
    if Config.HEROKU_MODE == "ENABLE":
        await event.reply("__Saat ini Mode Heroku DIAKTIFKAN sehingga Anda Tidak Dapat Melakukan Streaming Video karena Streaming Video Penyebab Banned Akun Heroku Anda__.")
        return
    ...
    if (
        ...
        return await event.client.send_file(chat_id, Config.CMD_IMG, caption="**Beri Saya Nama Lagu Yang Ingin Anda Mainkan**\n\n **Contoh**: `/vplay Febby Putri Usik`", buttons=btnn)
    if replied and not replied.video and not replied.document:
        xnxx = await event.reply("**🔄 Memproses Permintaan... Harap Tunggu!**")
        query = event.text.split(maxsplit=1)[1]
        search = ytsearch(query)
        RESOLUSI = 720
            ...
            if hm == 0:
                ...
                try:
                    ...
                    await event.client.send_file(event.chat_id,
                        f"**📡 Memulai Streaming 💡**\n\n💡 **Judul:** [{songname}]({url})\n**⏰ Durasi:** `{duration}`\n👥 **Permintaan Oleh:** {from_user}, buttons=btnn",
                        link_preview=False,
                    )
                except Exception as ep:
                    clear_queue(chat_id)
        ...
        xnxx = await event.reply("➕ **Mengunduh File Balasan**")
        dl = await replied.download_media()
        link = f"https://t.me/c/{chat.id}/{event.reply_to_msg_id}"
...
async def leavevc(event, perm):
    xnxx = await event.reply("Memproses")
    chat_id = event.chat_id
    from_user = vcmention(event.sender)
    if from_user:
        try:
            ...
            pass
        await xnxx.edit("**Meninggalkan Obrolan Suara** `{}`".format(str(event.chat_id)))
    else:
        await xnxx.edit(f"**Maaf {owner} tidak ada dalam obrolan suara**")
...
async def vc_pause(event, perm):
    ...
    if chat_id in QUEUE:
        try:
            ...
            await event.reply("**Streaming Dijeda**")
        except Exception as e:
            await event.reply(f"**ERROR:** `{e}`")
    else:
        await event.reply("**Tidak Ada Playing**")
...
async def vc_resume(event, perm):
    ...
    if chat_id in QUEUE:
        try:
            ...
            await event.reply("**Streaming Dimulai Kembali 🔙**")
        except Exception as e:
            await event.reply(f"**ERROR:** `{e}`")
    else:
        await event.reply("**Tidak Ada Streaming**")
